            ...
            >
                <Modal.Header>
                    {loading ? <Text>Se încarcă...</Text> : existingInvoice ?
                        <Col><Text h4>Vizualizează dovada plății</Text><Text>Poți descărca dovada pentru a vedea exact pentru ce
                            a fost efectuată plata</Text></Col> : <Text h4>Dovadă fiscală lipsă!</Text>}
                </Modal.Header>
                <Modal.Body>
                    {loading ? <Loading/> : existingInvoice ? <Button auto onPress={() => downloadInvoice()}>Descarcă</Button> :
                        <Text css={{textAlign: "center"}}>ONG-ul nu a încarcat încă o dovadă pentru a justifica această tranzacție!</Text>}
                </Modal.Body>
                <Modal.Footer>
